...
new class extends Component {
    ...
    // JSON Arrays
    // beneficios: [['icon' => '...', 'icon_mode' => 'text|image', 'icon_preview' => '...', 'titulo' => '...', 'descripcion' => '...']]
    public $beneficios = [];
    public $beneficioIconFiles = []; // separate array for file uploads
    ...
    {
        return 'Editar Beca: ' . $this->beca->nombre;
    }

    public function mount(Beca $beca)
    {
        $this->beca = $beca;
        ...
        // Normalize beneficios to always have icon_mode and icon_preview keys
        $rawBeneficios = is_array($beca->beneficios) ? $beca->beneficios : (json_decode($beca->beneficios, true) ?: []);
        $this->beneficios = array_map(function ($b) {
            return [
                'icon'         => $b['icon'] ?? '',
                'icon_mode'    => $b['icon_mode'] ?? 'text',
                'icon_preview' => '',  // previews are temp, reset on edit load
                'titulo'       => $b['titulo'] ?? '',
    ...
    public function update()
    {
        $this->validate([
            ...
            'slug' => 'required|string|max:255|unique:becas,slug,' . $this->beca->id,
...
<div>
    ...
    <div class="mb-12 flex justify-between items-end pb-4 border-b border-outline-variant/10">
        <div>
            ...
            <h2 class="text-4xl font-extrabold tracking-tight text-on-surface leading-none">Editar: {!! Str::limit($beca->nombre, 30) !!}</h2>
            <p class="text-on-surface-variant mt-2 max-w-xl">Modifica los detalles, requisitos y la información visible de la beca educativa.</p>
        </div>
                        ...
                        <div class="flex items-stretch gap-0 border-t border-outline-variant/10">
                            ...
                            <div class="flex flex-col shrink-0">
                                <button type="button"
                                    ...
                                    title="Icono de Material Symbols">
                            ...
                            <div class="flex-1 flex items-center px-4 py-3 border-l border-outline-variant/10">
                                ...
                                @else
                                    <div class="flex items-center gap-3 w-full">
                                        ...
                                        @elseif(!empty($beneficio['icon']) && $beneficio['icon_mode'] === 'image')
                                            <img src="{{ Storage::url($beneficio['icon']) }}" class="w-9 h-9 rounded-lg object-cover shrink-0 border border-outline-variant/20">
                                        @else
                                        ...
                                        <label class="flex-1 cursor-pointer">
                                            <span class="text-sm font-semibold text-primary hover:text-primary-dim transition-colors">
                                                {{ !empty($beneficio['icon_preview']) || (!empty($beneficio['icon']) && $beneficio['icon_mode'] === 'image') ? 'Cambiar imagen' : 'Seleccionar imagen del icono' }}
                                            </span>
                ...
                <div class="space-y-6">
                    ...
                    <div class="space-y-2 relative">
                        ...
                        <div class="relative overflow-hidden dashed-border aspect-video flex flex-col items-center justify-center p-6 text-center group cursor-pointer hover:bg-primary-container/10 transition-colors">
                            ...
                            @elseif ($imagen_portada_url)
                                <img src="{{ asset('storage/' . $imagen_portada_url) }}" class="absolute inset-0 w-full h-full object-cover">
                            @endif
                            <input type="file" wire:model="imagen_portada" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10">
                            
                            <div class="relative z-0 pointer-events-none {{ ($imagen_portada || $imagen_portada_url) ? 'opacity-0' : 'opacity-100' }}">
                    ...
                    <div class="space-y-2 relative">
                        ...
                        <div class="relative overflow-hidden dashed-border h-32 flex items-center justify-center p-4 text-center group cursor-pointer hover:bg-primary-container/10 transition-colors">
                            ...
                            <div class="relative z-0 pointer-events-none flex items-center gap-3 {{ ($banner || $banner_url) ? 'opacity-0' : 'opacity-100' }}">
...
<footer class="fixed bottom-0 right-0 w-full lg:w-[calc(100%-16rem)] h-20 glass-nav bg-white/90 border-t border-outline-variant/20 z-50 flex items-center justify-between px-6 lg:px-12">
    <div class="hidden md:flex items-center gap-4 text-xs font-medium text-on-surface-variant">
        <span class="flex items-center gap-1"><span class="w-2 h-2 rounded-full bg-secondary"></span> Editando</span>
        <span class="text-outline-variant">|</span>
        <span>Guardado por última vez hace poco</span>
    </div>
    <div class="flex items-center gap-4 ml-auto">
        ...
        <button wire:click="update" class="px-8 py-3 bg-primary text-on-primary rounded-xl font-bold shadow-xl shadow-primary/30 hover:bg-primary-dim hover:-translate-y-0.5 active:translate-y-0 transition-all flex items-center gap-2">
